package us.nuernberg.hackerrank.applesoranges
 ...
 *
 *  The second orange falls at position `15 - 6 = 6`.
 *
...
object Solution {
  ...
  def main(args: Array[String]): Unit = {
    ...
    println(input.appleHits)
    println(input.orangeHits)
  }

}

final case class House(start: Int, end: Int) {
  def contains(point: Int): Boolean = (point >= start) && (point <= end)
}

final case class Tree(root: Int, deltas: List[Int]) {
  val landingSpots: List[Int] = deltas.map(_ + root)
}

final case class Input(house: House, appleTree: Tree, orangeTree: Tree) {
  val hits: Tree => Int = _.landingSpots.count(house.contains)
  val appleHits: Int = hits(appleTree)
  val orangeHits: Int = hits(orangeTree)
}

object Input {
  def read(): Input = {
    val houseCoords :: treeCoords :: _ :: appleDeltas :: orangeDeltas :: Nil =
      io.Source.stdin.getLines.map(s => s.split(" ").toList.map(_.toInt)).toList
    val houseStart :: houseEnd :: Nil = houseCoords
    val appleStart :: orangeStart :: Nil = treeCoords
    Input(House(houseStart, houseEnd), Tree(appleStart, appleDeltas), Tree(orangeStart, orangeDeltas))
  }
}

object Test {
  def main(args: Array[String]): Unit = {
    val input = Input(House(7, 11), Tree(5, List(-2, 2, 1)), Tree(15, List(5, -6)))
    println(input.appleHits)
    println(input.orangeHits)
  }
}
