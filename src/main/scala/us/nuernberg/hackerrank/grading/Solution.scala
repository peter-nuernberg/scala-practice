package us.nuernberg.hackerrank.grading
...
object Solution {
  ...
  def main(args: Array[String]): Unit = {
    println(io.Source.stdin.getLines.drop(1).map(s => round(s.toInt)).mkString("\n"))
  }

  def round(raw: Int): Int =
    if ((raw >= 38) && (raw % 5 >= 3)) {
      ((raw / 5) + 1) * 5
    } else {
      raw
...
object Test {
  def main(args: Array[String]): Unit = {
    val grades = List(73, 67, 38, 33)
    println(grades.map(Solution.round))
  }
}
